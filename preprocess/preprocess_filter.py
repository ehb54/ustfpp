...
class DataPreprocessor:
    ...
    def _handle_negative_values(self, df: pd.DataFrame, model_name: str = None) -> pd.DataFrame:
        ...
        if negative_filter_mode == 'all_columns':
            self.logger.info(f"Filtering out rows with negative values in ALL numeric columns for {model_name or 'default'}")
            ...
            if valid_columns:
                self.logger.info(f"Filtering out rows with negative values in columns: {valid_columns} for {model_name or 'default'}")
    ...
    def process_for_single_model(self, df: pd.DataFrame, model_name: str, output_dir: str) -> Dict[str, Any]:
        ...
        output_filename = output_config.get('filename', f'{model_name}_filtered.csv')
    ...
    def process_file(self, input_file: str, output_dir: str = './results/filtered') -> Tuple[Optional[pd.DataFrame], List[Dict[str, Any]]]:
        ...
        try:
            ...
            metrics = self.process_for_single_model(df, model_name, output_dir)
